...
module {
  public let spec : ToolTypes.ToolSpec = {
    name = "wallet_buy_erc20_uniswap";
    description = "Buy ERC20 token on Uniswap V3 exactInputSingle. token_in must be pre-approved to router; amount_in and amount_out_minimum are base units.";
    parametersJson = "{\"type\":\"object\",\"properties\":{\"network\":{\"type\":\"string\",\"enum\":[\"ethereum\",\"base\",\"polygon\"]},\"router_address\":{\"type\":\"string\",\"description\":\"Uniswap V3 router address\"},\"token_in_address\":{\"type\":\"string\",\"description\":\"Input token (ERC20) address\"},\"token_out_address\":{\"type\":\"string\",\"description\":\"Output token (ERC20) address\"},\"fee\":{\"type\":\"integer\",\"minimum\":1,\"description\":\"Pool fee tier (e.g. 500, 3000, 10000)\"},\"amount_in\":{\"type\":\"integer\",\"minimum\":1,\"description\":\"Input token amount in base unit\"},\"amount_out_minimum\":{\"type\":\"integer\",\"minimum\":0,\"description\":\"Minimum output amount in base unit\"},\"deadline\":{\"type\":\"integer\",\"minimum\":1,\"description\":\"Unix timestamp seconds; default now + 1200\"},\"sqrt_price_limit_x96\":{\"type\":\"integer\",\"minimum\":0,\"description\":\"Set 0 for no limit\"}},\"required\":[\"network\",\"router_address\",\"token_in_address\",\"token_out_address\",\"fee\",\"amount_in\",\"amount_out_minimum\"],\"additionalProperties\":false}";
    argNames = [
      "network",
      "router_address",
      "token_in_address",
  ...
  public func run(args : [Text], deps : ToolTypes.DispatchDeps) : async ToolTypes.ToolResult {
    if (args.size() < 7) {
      return #err(
        "wallet_buy_erc20_uniswap requires args: network, router_address, token_in_address, token_out_address, fee, amount_in, amount_out_minimum",
      );
    };
